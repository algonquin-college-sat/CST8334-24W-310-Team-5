...
class Impl_DatasetsWindow(Ui_DatasetsWindow, QtWidgets.QMainWindow,):
    ...
    def saveDatasetFile(self, fileName):
        ...
        with open(self.saved_dataset_path, 'r') as file:
            data = file.read()
...
# adjust
    ...
    def cBox_Preset_currentTextChanged(self):
        ...
        elif currText == "SonarQube (XML)":
            self.tbl_Dataset.setRowCount(0)
            self.datasetColumns = []
            for i in range(len(SONARQUBE_PRESET_XML)):
                if i == 0:
                    # First is output column
                    dsCol = DatasetColumn(
                        SONARQUBE_PRESET_XML[i],
                        "Categorical",
                        True,
                        "One-hot Encoding",
                        "Output",
        ...
        elif currText == "Pytest (XML)":
            self.tbl_Dataset.setRowCount(0)
            self.datasetColumns = []
            for i in range(len(PYTEST_PRESET_XML)):
                if i == 0:
                    # First is output column
                    dsCol = DatasetColumn(
                        PYTEST_PRESET_XML[i],
        ...
        elif currText == "Pylint (CSV)":
            self.tbl_Dataset.setRowCount(0)
            self.datasetColumns = []
            for i in range(len(PYLINT_PRESET_CSV)):
                if i == 0:
                    # First is output column
                    dsCol = DatasetColumn(
                        PYLINT_PRESET_CSV[i],
        ...
        elif currText == "ESLint (XML)":
            self.tbl_Dataset.setRowCount(0)
            self.datasetColumns = []
            for i in range(self.cBox_Root.count()):
                if self.cBox_Root.itemText(i) == ESLINT_PRESET_XML:
                    self.cBox_Root.setCurrentIndex(i)
                    break
            for i in range(len(ESLINT_PRESET_XML)):
                if i == 0:
                    # First is output column
                    dsCol = DatasetColumn(
                        ESLINT_PRESET_XML[i],
        ...
        elif currText == "JSLint (XML)":
            self.tbl_Dataset.setRowCount(0)
            self.datasetColumns = []
            for i in range(self.cBox_Root.count()):
                if self.cBox_Root.itemText(i) == JSLINT_PRESET_XML:
                    self.cBox_Root.setCurrentIndex(i)
                    break
            for i in range(len(JSLINT_PRESET_XML)):
                if i == 0:
                    # First is output column
                    dsCol = DatasetColumn(
                        JSLINT_PRESET_XML[i],
