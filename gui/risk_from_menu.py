...
class Impl_LabelerToRisk(
    ...
    def __init__(self):
        ...
        super(Impl_LabelerToRisk, self).__init__()
        self.setupUi(self)
        # Prompt the user to select a dataset path
        self.path = self.promptForDatasetPath()
    
        if self.path:  # Proceed only if a path was selected
            self.customInit()
            self.customEvents()
            self.loadDatasetFromCall(self.path)
            self.btn_Risk_clicked()
            
        else:
            # Handle the case where no path is selected, if needed
            pass
    ...
    def promptForDatasetPath(self):
        """Opens a file dialog to prompt the user to select a dataset path."""
